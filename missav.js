        ...
        },
        {
            ...
            params: [
                ...
                { name: "sort_by", title: "endpoint", type: "enumeration", value: "dm179/cn/actresses/%E7%80%AC%E6%88%B8%E7%92%B0%E5%A5%88", enumOptions: [
                    ...
                    { title: "初美なのか", value: "dm179/cn/actresses/%E5%88%9D%E7%BE%8E%E3%81%AA%E3%81%AA%E3%81%AE%E3%81%8B" },
...
function extractVideoId(href) {
    return (href.split('/').pop() || "").replace(/-(uncensored-leak|chinese-subtitle)$/, "").toUpperCase();
}

function resolveUrl(path) {
    return !path ? "" : (path.startsWith("http") ? path : `${BASE_URL}${path.startsWith("/") ? path : `/${path}`}`);
}
...
async function loadDetail(link) {
    try {
        const { data: html } = await Widget.http.get(link, { headers: HEADERS });
        const $ = Widget.html.load(html);
        ...
        ['a[href*="/actresses/"]', 'a[href*="/actors/"]', 'a[href*="actresses"]', 'a[href*="actors"]'].forEach(selector => {
            $(selector).each((_, el) => {
                ...
                const img = $el.find('img').attr('data-src') ?? $el.find('img').attr('src') ?? $el.closest('div,li,figure').find('img').attr('data-src') ?? $el.closest('div,li,figure').find('img').attr('src') ?? "";
                if (name) pushActor(name, img, resolveUrl($el.attr('href')));
            });
        });
        ...
        $('script').each((_, el) => {
            ...
            if (content.includes('surrit.com') && content.includes('.m3u8')) {
                videoUrl = content.match(/https:\/\/surrit\.com\/[a-f0-9\-]+\/[^"'\s]*\.m3u8/)?.[0] ?? videoUrl;
                if (videoUrl) return false;
            }
            if (!videoUrl && content.includes('eval(function')) {
                const uuid = content.match(/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/)?.[0];
                if (uuid) {
